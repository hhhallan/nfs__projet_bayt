...
if (isset($_POST['submit'])) {
    ...
    if (!empty(['name', 'firstname', 'tel', 'tel2', 'email', 'password', 'password_comfirm'])) {
        extract($_POST);
        if (mb_strlen($name) < 3) {
        ...
        if (empty($error)) {
            ...
            try {
                $saveIsOk = $bdd->prepare(
                    'INSERT INTO user_part(nom, prenom, email, mot_de_passe,tel_portable,tel_fixe) 
                                        VALUE(:nom, :prenom, :email, :mot_de_passe, :tel_portable,:tel_fixe)',
                    [
                        'nom'     => $name,
                        'prenom'   => $firstname,
                        'email'    => $email,
